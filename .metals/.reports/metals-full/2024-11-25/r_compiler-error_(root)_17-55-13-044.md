file://<WORKSPACE>/src/main/scala/routes.scala
...
action parameters:
offset: 3690
uri: file://<WORKSPACE>/src/main/scala/routes.scala
text:
```scala
...
case class Controller(
  ...
  private def createRoom(input: RoomInput): Future[ToResponseMarshallable]= 
    rooms
    .ask[Option[ActorRef[Message]]]( ref =>CreateRoom(input.name))
    .map{
      case Some(room)=>
        StatusCodes.Created -> input.name
      case None =>
  ...
  private def listRooms(): Future[ToResponseMarshallable] = 
    rooms
    .ask[Option[Map[String, ActorRef[Message]]]](ref => ListRooms(ref))
    .map{
      case Some(room)=>
        StatusCodes.OK  -> rooms[@@]
      case None =>
  ...
  private def createPost(roomId: String, input: PostInput): Future[ToResponseMarshallable]= 
    ...
    .map{
      case Some(post)=>
        StatusCodes.Created -> post.output(roomId)
      case None =>
        StatusCodes.BadRequest
    }
